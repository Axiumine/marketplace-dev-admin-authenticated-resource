...
// them (GraphQLIndirizzoBaseFrag, GraphQLPositionFrag, GraphQLInputAnagraficaImprenditore,
// GraphQLInputLogin), so they have to see the same transformed graphql copy as the sources.
// The bare /graphql/ pattern already covers graphql-scalars, graphql-upload and
// graphql-depth-limit, which build scalars and validation rules from the same package.
const inlineDeps = [
	/graphql/,
	/@apollo\/server/,
	/@as-integrations/,
...
//   - integration: boots the real server against the real Redis cluster AND a real MongoDB.
//                  Redis params come from .env via the sources' own dotenv.config(); the keyspace
//                  prefix is pinned to an isolated, ACL-allowed namespace and PORT=0 is ephemeral.
//                  MONGODB_URI is NOT the one in .env: it is rebuilt from the MONGO_TEST_* block
//                  so the service talks to its own throwaway database with the least-privilege
//                  R/W user, while globalSetup provisions that database as the DB owner.
		...
		projects: [
			...
			},
			{
				...
				test: {
					...
					env: {
						NODE_ENV: 'test',
						REDIS_KEY: 'marketplaceDev:itest:adminAuthenticatedResource:',
						INTROSPECTION_CODE: 'test-introspection-code',
						PORT: '0',
