...
export const keygripRotate = {
	 ...
	 * account on the platform. The admin asks for a rotation; they do not get to say what it produces.
	 *
	 * ⚠️ `Boolean!` for the same reason: the answer is "it happened", never the keys. Everything an
	 * admin needs to see afterwards — the version, the fingerprint, which service has adopted it — is
	...
	async resolve(_: unknown, __: unknown, ctx: IContextAdminAuthenticatedResource) {
		try {
			// The admin's id is the session's, and travels only into the audit event.
