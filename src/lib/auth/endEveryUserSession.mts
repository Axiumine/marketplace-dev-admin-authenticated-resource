 ...
 * so everything `endEveryShopOwnerSession` documents holds here unchanged: the operator's own session is
 * untouched — E15-S05's "the caller goes too" is a rule about whose credentials changed, and these are not
 * the caller's — no access key is deleted by name because the operator does not hold the customer's token,
 * and the residual is one access-token lifetime, since refresh sessions die here while an access token
 * already issued lives out its own expiry.
 *
