...
/**
 * The imprenditori-over-time series behind the operator's stats chart.
 *
 ...
 *
 * ⚠️ **Unfiltered, exactly like `imprenditoriStatsDb`.** No `deleted` / `disabled` exclusion, so the
 ...
 * is UTC+1/+2, so an imprenditore registered at 00:30 local lands in the previous day's bucket. That
 ...
 *
 * `granularita` and `format` are NOT asked for by the client and not stored apart from `months`,
 ...
 */
const RANGES = {
	TUTTO: { months: null, granularita: 'MESE', format: '%Y-%m-01' },
	TRE_MESI: { months: 3, granularita: 'GIORNO', format: '%Y-%m-%d' },
	UN_MESE: { months: 1, granularita: 'GIORNO', format: '%Y-%m-%d' }
} as const

export type PeriodoImprenditori = keyof typeof RANGES

export type GranularitaPeriodo = (typeof RANGES)[PeriodoImprenditori]['granularita']

/**
 * `data` is a `YYYY-MM-DD` string in **both** granularities — a month bucket is stamped with its own
 * first day rather than truncated to `YYYY-MM`. One format means the client has one parse path, and
 * `granularita` tells it how to label the axis. The alternative, a string whose meaning depends on
 * its length, invites exactly the off-by-one nobody notices until December.
 */
export interface IImprenditoriPerPeriodoPunto {
	data: string
	totale: number
}

export interface IImprenditoriPerPeriodo {
	granularita: GranularitaPeriodo
	punti: IImprenditoriPerPeriodoPunto[]
}
 ...
 * imprenditore: the aggregation has already visited every document, so asking again would be a full
 ...
 * no imprenditori has no oldest month, so the loop below never starts and the series is empty — an
 * axis drawn from nothing would have to invent its own endpoints, and a chart showing a flat zero
 * line for a platform with no imprenditori is indistinguishable from one whose query failed. An
...
function monthBuckets(counts: Map<string, number>, to: number): number[] {
	...
	for (const primo of [...counts.keys()].sort().slice(0, 1)) {
		let ms = startOfMonthUTC(fromBucketKey(primo))
 ...
 * produces no group at all — and are filled in by `serie`.
 */
async function countsByBucket(from: number | null, format: string): Promise<Map<string, number>> {
	const pipeline: PipelineStage[] = []

	// Only the bounded ranges get a `$match`, and it is what the `{ iscrizione: 1 }` index in
	...
	if (from !== null) {
		pipeline.push({ $match: { iscrizione: { $gte: new Date(from) } } })
	}
	...
	const rows = await Imprenditore.aggregate<{ _id: string; totale: number }>(pipeline)

	return new Map(rows.map((r) => [r._id, r.totale]))
}

/** Gap filling: every bucket in the range gets a point, and a bucket nobody registered in gets 0. */
function serie(buckets: number[], counts: Map<string, number>): IImprenditoriPerPeriodoPunto[] {
	return buckets.map((ms) => {
		const data = toBucketKey(ms)

		return { data, totale: counts.get(data) ?? 0 }
	})
}
 ...
 * `monthBuckets`.
 */
export default async function imprenditoriPerPeriodoDb(periodo: PeriodoImprenditori): Promise<IImprenditoriPerPeriodo> {
	const { months, granularita, format } = RANGES[periodo]
	const oggi = startOfDayUTC(new Date())

	if (months === null) {
		const counts = await countsByBucket(null, format)

		return { granularita, punti: serie(monthBuckets(counts, oggi), counts) }
	}

	const from = subMonthsUTC(oggi, months)
	const counts = await countsByBucket(from, format)

	return { granularita, punti: serie(dayBuckets(from, oggi), counts) }
}
