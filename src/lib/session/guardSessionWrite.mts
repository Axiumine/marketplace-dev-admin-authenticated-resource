...
/** One hour, in seconds — the window an operator's session revocations are metered over. */
export const SESSION_WRITE_WINDOW_SECONDS = 3600

/**
 * Session revocations one operator may make per hour, per operation.
 *
 * Sixty, an order of magnitude above the keygrip figure and for the opposite reason: ending sessions is the
 * routine half of an incident — an operator working through a list of compromised accounts issues one call
 * per account — while rotating the platform's signing keys is a monthly job. The cap is here to stop a
 * runaway client, not to ration the operator's judgement.
 *
 ...
 * every session on the platform, not an operator who has to wait.
 */
export const SESSION_WRITES_PER_HOUR = 60

/**
 * Meters one operator's session revocations (E17-S03).
 *
 * ⚠️ **The identity is the operator's account id, never a network address** (BCON-01). `app.proxy` is off
 ...
 * id into the key, so the counters do not become a list of which operators ended whose sessions.
 *
 ...
 */
export async function guardSessionWrite(operation: string, operator: string) {
