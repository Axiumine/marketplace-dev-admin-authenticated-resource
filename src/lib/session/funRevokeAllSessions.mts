...
/**
 * Ends every session one account holds, from the admin's console, and answers how many there
 * were so the console can say what it just did rather than "done".
 *
 * ⚠️ **The routine is `revokeAllSessionsForAccount`, imported and not re-implemented.** It is the same code a password change,
 * a disable and a status change revoke through, including its re-read of the index before the key is
 ...
 * holding the caller's session can aim at — the same reasoning `endEverySession` records.
 */
