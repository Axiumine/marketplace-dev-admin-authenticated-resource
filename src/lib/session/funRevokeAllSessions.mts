...
/**
 * Ends every session one account holds, from the operator's console (E17-S03), and answers how many there
 ...
 * that fails open, on the one path where an operator has been told the account is closed.
 *
 ...
 * sessions — which on this path means an operator investigating one account logging out another.
 *
 * ⚠️ **This ends the operator's own sessions when they name their own account.** That is not guarded
