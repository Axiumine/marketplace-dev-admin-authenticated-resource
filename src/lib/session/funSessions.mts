...
/**
 * One live session, as an admin needs to see it.
 *
 ...
 * That is the standing decision this platform has taken on session rows, and the reason the row
...
/**
 * Lists the sessions one account holds.
 *
 * ⚠️ **The index is read with `hGetAll` on one key — never `SCAN`, never `KEYS`** (BCON-08). One account's
 * sessions are one hash: one key, one round trip, one slot on a cluster. This is the whole reason the session index
 * was built in the first place, and a console that scanned would be the thing it was built to avoid.
 *
 ...
 * their own field and the index gives every field a TTL, but a session whose key expired between that TTL and
