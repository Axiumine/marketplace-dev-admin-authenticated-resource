...
/**
 * The cap on an admin's suspension note, and **the only place it is enforced on the platform**.
 *
 ...
 * "encrypt the admin's prose about a person, or count its characters".
 */
export const MAX_DISABLED_REASON = 1000

/**
 * The reason an admin gives for a suspension: required when suspending, discarded when releasing.
 *
 ...
 * text in its textarea while the admin unticks the box is describing "not suspended", and answering
 ...
 * and the missing argument are one case here — an admin who suspends somebody owes a sentence, and
