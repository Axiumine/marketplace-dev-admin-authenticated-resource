 ...
 *
 * ⚠️ **Parameterised by model rather than copied per collection, and that is the one thing to know
 * before adding a third caller.** `shopOwnersPerPeriodDb` and `usersPerPeriodDb` are both two lines
 * over this file. The sibling pair `shopOwnersActiveTblDb` / `usersActiveTblDb` went the other way and
 * stayed two files, for a reason that does not apply here: those two DIVERGE — the customers table has
 * no `search` at all, because every field the shop-owner one searches is random ciphertext on `user`
 * (ADR-029). Nothing in this file touches an encrypted field. `registeredAt`, `deleted` and `disabled`
 * are clear on both collections, and this query reads only the first of the three, so the two callers
 * are not merely similar today — they have no axis to differ along. Duplicating it would have meant two
 * copies of `subMonthsUTC`'s month-length clamp, which is the kind of arithmetic that is wrong in one
 * copy for a year before anybody notices in February.
 *
 * ⚠️ **Unfiltered, exactly like the `*StatsDb` count each chart sits under.** No `deleted` / `disabled`
 * exclusion, so the points sum to the "Total" row above them. Filtering here and not there would put
 * two numbers that look like the same number on the same screen, disagreeing. Since ADR-041 a closed
 * account keeps its document for ever, so a past bucket's height no longer changes thirty days later
 * either — under the TTL it did.
 *
 * ⚠️ **Everything is bucketed in UTC**, both in the `$dateToString` below and in the gap filling. Someone
 * in a zone ahead of UTC who registers at 00:30 local lands in the previous day's bucket. That is a
 * deliberate trade: bucketing in a local zone means every boundary — the range start, the day steps, the
 * month steps — has to be computed in a zone with two DST transitions a year, and a one-to-two-hour skew
 * on a registration timestamp is invisible at this chart's resolution. If a chart ever gains an
 * hour-of-day axis, this is the decision to revisit first.
 */

/**
 * One entry per range the GraphQL enums offer, holding all three facts about it together.
 *
...
export type StatsPeriod = keyof typeof RANGES

export type PeriodGranularity = (typeof RANGES)[StatsPeriod]['granularity']
 ...
 */
export interface IPerPeriodPoint {
	date: string
	total: number
}

export interface IPerPeriod {
	granularity: PeriodGranularity
	points: IPerPeriodPoint[]
}

/**
 * All this file needs of a model, spelled structurally rather than as `Model<IUserModel>`: the two
 * callers hand it two unrelated document types, and nothing here reads a field off a document — the
 * `$group` returns `{ _id, total }` whatever it aggregated. `PromiseLike` rather than `Promise`
 * because mongoose answers an `Aggregate`, which is thenable and not a `Promise`.
 */
export interface ISeriesModel {
	aggregate<T>(pipeline: PipelineStage[]): PromiseLike<T[]>
}
 ...
 * document: the aggregation has already visited every one of them, so asking again would be a full
 ...
 * nobody registered has no oldest month, so the loop below never starts and the series is empty — an
 * axis drawn from nothing would have to invent its own endpoints, and a chart showing a flat zero
 * line for an empty collection is indistinguishable from one whose query failed. An explicit early
 * return for that case is unobservable: the general path already answers `[]`, so removing the guard
 * changes no output, and a guard nothing can observe is a guard no test can hold in place. As a
 * one-element seed list it is the same decision, expressed where it can be checked.
 *
 ...
 */
async function countsByBucket(model: ISeriesModel, from: number | null, format: string): Promise<Map<string, number>> {
	const pipeline: PipelineStage[] = []

	// Only the bounded ranges get a `$match`, and it is what the single-field `{ registeredAt: 1 }`
	// index serves — `registeredAt_series`, built by `20260301000100` on `shopOwner` and by
	// `20260829000200` on `user` in marketplace-db-setup. `ALL` reads the whole collection by
	// definition, so a stage matching everything would only add work.
	if (from !== null) {
	...
	const groups = await model.aggregate<{ _id: string; total: number }>(pipeline)
...
function series(buckets: number[], counts: Map<string, number>): IPerPeriodPoint[] {
 ...
 * `monthBuckets`.
 */
export default async function registeredAtSeriesDb(model: ISeriesModel, period: StatsPeriod): Promise<IPerPeriod> {
	const { months, granularity, format } = RANGES[period]
	const today = startOfDayUTC(new Date())

	if (months === null) {
		const counts = await countsByBucket(model, null, format)

		return { granularity, points: series(monthBuckets(counts, today), counts) }
	}

	const from = subMonthsUTC(today, months)
	const counts = await countsByBucket(model, from, format)
