...
describe('usersActiveTblDb', () => {
	...
	describe('the three filters', () => {
		...
		it('defaults to the live, enabled accounts and counts exactly what it lists', async () => {
			...
			expect(filterOf(find)['emailVerify.valid']).toBeUndefined()
		...
		it('keys the filter in the order the index is keyed', async () => {
			...
			expect(Object.keys(filterOf(find))).toEqual(['deleted', 'disabled', 'emailVerify.valid'])
		})
		...
		it.each([
			...
			expect(filterOf(find)['emailVerify.valid']).toEqual(expected)
		})
