...
vi.mock('@thedoctorweb_agency/marketplace-common/models/MongoDB/Imprenditore', () => ({
	Imprenditore: { find, countDocuments }
}))

const { default: imprenditoriAttiviTblDb } = await import('../src/lib/imprenditore/imprenditoriAttiviTblDb.mts')

// Spelled out rather than imported from the module under test. Asserting against
// IMPRENDITORI_TBL_SELECTION is a tautology — it compares the constant with itself, so emptying it
...
const SELECTION = '_id iscrizione anagrafica.nome anagrafica.cognome anagrafica.indirizzo'

type Args = Parameters<typeof imprenditoriAttiviTblDb>[0]
...
function args(overrides: Partial<Args> = {}): Args {
	return { offset: 0, limit: 25, sortBy: 'ISCRIZIONE', sortDir: 'DESC', ...overrides }
}
...
describe('imprenditoriAttiviTblDb', () => {
	beforeEach(() => {
		find.mockReset()
		countDocuments.mockReset().mockResolvedValue(0)
	})

	it('pages the active imprenditori and reports the size of the filtered set', async () => {
		const docs = [{ nome: 'Mario' }]
		const builder = mockFind(docs)
		countDocuments.mockResolvedValueOnce(137)

		await expect(imprenditoriAttiviTblDb(args({ offset: 50, limit: 10 }))).resolves.toEqual({
			items: docs,
			total: 137
		})
	...
	it('excludes the disabled and the soft-deleted, and counts exactly what it lists', async () => {
		...
		await imprenditoriAttiviTblDb(args())
	...
	describe('search', () => {
		it('matches a case-insensitive prefix across the three text columns', async () => {
			...
			await imprenditoriAttiviTblDb(args({ search: 'ros' }))

			expect(filterOf().$or).toEqual([
				{ 'anagrafica.nome': /^ros/i },
				{ 'anagrafica.cognome': /^ros/i },
				{ 'anagrafica.indirizzo.comune': /^ros/i }
			])

			const [{ 'anagrafica.nome': regex }] = filterOf().$or
		...
		it('escapes every regex metacharacter in the term', async () => {
			...
			await imprenditoriAttiviTblDb(args({ search: 'a.b*c+d?e^f$g{h}i(j)k|l[m]n\\o' }))
		...
		it('anchors the match, so a substring in the middle of a name is not a hit', async () => {
			...
			await imprenditoriAttiviTblDb(args({ search: 'ossi' }))
		...
		it('trims the term before using it', async () => {
			...
			expect(filterOf().$or[0]).toEqual({ 'anagrafica.nome': /^ros/i })
		})
		...
		it.each([
			...
			await imprenditoriAttiviTblDb(args({ search }))
		...
		it('accepts a term of exactly the maximum length', async () => {
			...
			await expect(imprenditoriAttiviTblDb(args({ search: 'a'.repeat(100) }))).resolves.toBeDefined()
		})

		it('rejects a term one character past the maximum', async () => {
			await expect(imprenditoriAttiviTblDb(args({ search: 'a'.repeat(101) }))).rejects.toMatchObject({
				message: 'Bad Request',
	...
	describe('sorting', () => {
		...
		it.each([
			['ISCRIZIONE', 'DESC', { iscrizione: -1, _id: -1 }],
			['ISCRIZIONE', 'ASC', { iscrizione: 1, _id: 1 }],
			['NOME', 'DESC', { 'anagrafica.nome': -1, _id: -1 }],
			['NOME', 'ASC', { 'anagrafica.nome': 1, _id: 1 }],
			['COGNOME', 'DESC', { 'anagrafica.cognome': -1, 'anagrafica.nome': -1, _id: -1 }],
			['COGNOME', 'ASC', { 'anagrafica.cognome': 1, 'anagrafica.nome': 1, _id: 1 }],
			['COMUNE', 'DESC', { 'anagrafica.indirizzo.comune': -1, _id: -1 }],
			['COMUNE', 'ASC', { 'anagrafica.indirizzo.comune': 1, _id: 1 }]
		])('sorts by %s %s', async (sortBy, sortDir, expected) => {
			const builder = mockFind([])

			await imprenditoriAttiviTblDb(args({ sortBy: sortBy as Args['sortBy'], sortDir: sortDir as Args['sortDir'] }))

			expect(builder.sort).toHaveBeenCalledExactlyOnceWith(expected)
		})

		// Key ORDER, not just presence: a sort document is ordered, so `{_id, iscrizione}` is a
		// different query from `{iscrizione, _id}` — and only the second one an index can serve.
		it('breaks ties on _id, last', async () => {
			const builder = mockFind([])

			await imprenditoriAttiviTblDb(args({ sortBy: 'COGNOME' }))

			expect(Object.keys(builder.sort.mock.calls[0][0])).toEqual(['anagrafica.cognome', 'anagrafica.nome', '_id'])
		})
	})
	...
	describe('paging bounds', () => {
		it.each([
			...
			await expect(imprenditoriAttiviTblDb(args(overrides))).rejects.toMatchObject({
				message: 'Bad Request',
		...
		it.each([
			...
			await expect(imprenditoriAttiviTblDb(args(overrides))).resolves.toBeDefined()
		})
		...
		it('accepts an offset far past the end of the collection', async () => {
			...
			await expect(imprenditoriAttiviTblDb(args({ offset: 1_000_000 }))).resolves.toBeDefined()
		})
	})
})
