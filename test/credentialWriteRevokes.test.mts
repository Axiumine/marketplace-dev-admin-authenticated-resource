...
/*
 * ⚠️ **The enumeration: every credential write in this service ends the account's
 * sessions, and a new one cannot be added quietly.** The three tests above it — one per mutation —
 * prove the call sites that exist today. This one proves there are no *others*: it walks `src/`, finds
 * every file that writes a login identifier, and fails on any writer whose resolver does not revoke.
 *
 ...
 * it is where the platform writes *to* a person, not what they sign in with.
 */

const SRC = new URL('../src/', import.meta.url).pathname
const MUTATIONS = join(SRC, 'graphQLApi/schema/mutations')

// The writers this service is known to hold. The assertion is equality, not inclusion: a tenth mutation
// that writes a credential lands here as a failing test naming the file, which is the point of the story.
const KNOWN_WRITERS = ['src/lib/admin/funAdminUpdatePwd.mts', 'src/lib/shopOwner/funShopOwnerUpdateEmail.mts']
