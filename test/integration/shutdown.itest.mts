 ...
 * connections destroyed below are this file's, not the ones index.itest.mts is using.
 */
...
describe('production hardening actually applies to a real server', () => {
	...
	it('mirrors the listening banner to Sentry under production', () => {
		...
		try {
			logListening({ NODE_ENV: 'production', PORT: '4024' })

			expect(info).toHaveBeenCalledWith(`Serving http://*:4024${ENDPOINT} for production.`)
		} finally {
			info.mockRestore()
		}
	})
...
describe('process-level error handlers', () => {
	...
	// Deliberately non-fatal on this tier (see the comment on onUnhandledRejection in src/index.mts):
	// an upload rejecting late must not take the whole resource server down. Asserting exit was NOT
	// called is what actually distinguishes this from onUncaughtException below — both report to
	// Sentry, only one of them kills the process.
	it('reports an unhandled rejection without exiting the process', () => {
