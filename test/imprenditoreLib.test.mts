...
vi.mock('@thedoctorweb_agency/marketplace-common/models/MongoDB/Imprenditore', () => ({
	Imprenditore: { updateOne, countDocuments }
}))

const { funImprenditoreDelete } = await import('../src/lib/imprenditore/funImprenditoreDelete.mts')
const { funImprenditoreUpdate } = await import('../src/lib/imprenditore/funImprenditoreUpdate.mts')
const { funImprenditoreUpdateEmail } = await import('../src/lib/imprenditore/funImprenditoreUpdateEmail.mts')
const { funImprenditoreUpdatePreferenze } = await import('../src/lib/imprenditore/funImprenditoreUpdatePreferenze.mts')
const { funImprenditoreUpdateNote } = await import('../src/lib/imprenditore/funImprenditoreUpdateNote.mts')
const { funImprenditoreUpdateStato } = await import('../src/lib/imprenditore/funImprenditoreUpdateStato.mts')
const { default: imprenditoriStatsDb } = await import('../src/lib/imprenditore/imprenditoriStatsDb.mts')
...
const anagrafica = {
	nome: 'Mario',
	cognome: 'Rossi',
	indirizzo: { via: 'Via Roma 1', cap: '20100', citta: 'Milano', provincia: 'MI', nazione: 'IT' }
} as never
 ...
 * is the case the newer helpers deliberately treat as success — see `funImprenditoreUpdateStato`.
 */
...
describe('funImprenditoreDelete', () => {
	beforeEach(() => updateOne.mockReset())

	// Soft delete: the document stays, `deleted` gets a timestamp and the approval flag is dropped
	// so a deleted imprenditore cannot reappear in the "waiting for approval" queue.
	it('stamps deleted and clears waitApprov', async () => {
		mockUpdate(1)

		await expect(funImprenditoreDelete(_id)).resolves.toBeUndefined()
	...
	it('raises a 500 when the imprenditore did not exist', async () => {
		mockUpdate(0)

		await expect(funImprenditoreDelete(_id)).rejects.toThrow('Internal Server Error')
	})
})

describe('funImprenditoreUpdate', () => {
	beforeEach(() => updateOne.mockReset())

	it('replaces the whole anagrafica sub-document', async () => {
		mockUpdate(1)

		await expect(funImprenditoreUpdate(_id, anagrafica)).resolves.toBeUndefined()

		expect(updateOne).toHaveBeenCalledExactlyOnceWith({ _id }, { $set: { anagrafica } })
	})

	// modifiedCount is 0 both when nothing matched and when the update was a no-op, so an
	// unchanged anagrafica is reported as a failure. Pinned as current behaviour.
	it('raises a 500 when nothing was modified', async () => {
		mockUpdate(0)

		await expect(funImprenditoreUpdate(_id, anagrafica)).rejects.toThrow('Internal Server Error')
	})
})

describe('funImprenditoreUpdateEmail', () => {
	beforeEach(() => updateOne.mockReset())

	it('writes the dotted login.email path, leaving the rest of login alone', async () => {
		mockUpdateMatched(1)

		await expect(funImprenditoreUpdateEmail(_id, 'nuova@marketplace.test')).resolves.toBeUndefined()
		...
		expect(updateOne).toHaveBeenCalledExactlyOnceWith({ _id }, { $set: { 'login.email': 'nuova@marketplace.test' } })
	})

	// The address is the collection's one unique index, so this is the only write on the imprenditore
	...
	it('turns the unique-index violation into a 409 naming the field', async () => {
		...
		expect(await rejection(funImprenditoreUpdateEmail(_id, 'presa@marketplace.test'))).toEqual({
			message: 'Conflict',
			http: { status: 409 },
			description: 'email: indirizzo già registrato da un altro imprenditore'
		})
	})
	...
	it('rethrows any other driver error untouched', async () => {
		const guasto = Object.assign(new Error('connessione persa'), { code: 'ECONNRESET' })
		updateOne.mockReturnValueOnce({ exec: vi.fn().mockRejectedValue(guasto) })

		await expect(funImprenditoreUpdateEmail(_id, 'nuova@marketplace.test')).rejects.toBe(guasto)
	})

	it('raises a 404 when no imprenditore carries that id', async () => {
		mockUpdateMatched(0)

		expect(await rejection(funImprenditoreUpdateEmail(_id, 'nuova@marketplace.test'))).toEqual({
			message: 'Oops',
			http: { status: 404 },
...
describe('funImprenditoreUpdateStato', () => {
	beforeEach(() => updateOne.mockReset())
	...
	it.each([
		...
		await expect(funImprenditoreUpdateStato(_id, disabled, waitApprov)).resolves.toBeUndefined()
	...
	// the divergence from funImprenditoreUpdate's `modifiedCount !== 1` 500 above is deliberate.
	it('accepts a write that changed nothing, as long as the document exists', async () => {
		...
		await expect(funImprenditoreUpdateStato(_id, true, true)).resolves.toBeUndefined()
	})

	it('raises a 404 when no imprenditore carries that id', async () => {
		mockUpdateMatched(0)

		expect(await rejection(funImprenditoreUpdateStato(_id, true, true))).toEqual({
			message: 'Oops',
			http: { status: 404 },
			description: 'imprenditore non trovato'
		})
...
describe('funImprenditoreUpdateNote', () => {
	beforeEach(() => updateOne.mockReset())

	// Top level, not inside `anagrafica`: the path carries no prefix, unlike the three login
	// preferences below. The note is what an operator wrote *about* the account.
	it('stores a note at the root of the document', async () => {
		mockUpdateMatched(1)

		await expect(funImprenditoreUpdateNote(_id, 'Richiamare a settembre')).resolves.toBeUndefined()

		expect(updateArgs()).toEqual({ filter: { _id }, update: { $set: { note: 'Richiamare a settembre' } } })
	})

	// The empty string removes the field rather than storing `''`, exactly as `false` removes a flag in
	// funImprenditoreUpdateStato above: the collection makes `note` optional, so "no note" already has
	...
	it('unsets the note when it is cleared', async () => {
		...
		await expect(funImprenditoreUpdateNote(_id, '')).resolves.toBeUndefined()

		expect(updateArgs()).toEqual({ filter: { _id }, update: { $unset: { note: 1 } } })
	})

	// `matchedCount`, like the two helpers above and unlike funImprenditoreUpdate: re-saving the note
	// an account already carries is the state the operator asked for, not a failure.
	it('accepts a write that changed nothing, as long as the document exists', async () => {
		updateOne.mockReturnValueOnce({ exec: vi.fn().mockResolvedValue({ matchedCount: 1, modifiedCount: 0 }) })

		await expect(funImprenditoreUpdateNote(_id, 'invariata')).resolves.toBeUndefined()
	})

	it('raises a 404 when no imprenditore carries that id', async () => {
		mockUpdateMatched(0)

		expect(await rejection(funImprenditoreUpdateNote(_id, 'x'))).toEqual({
			message: 'Oops',
			http: { status: 404 },
			description: 'imprenditore non trovato'
		})
...
describe('funImprenditoreUpdatePreferenze', () => {
	beforeEach(() => updateOne.mockReset())

	// Every path is prefixed `login.`: these three live inside the login sub-document, not at the root.
	// Both booleans are stored as they are — false included — unlike the two stato flags above.
	it.each([
		...
		await expect(funImprenditoreUpdatePreferenze(_id, rememberMe, onboardingDone, step)).resolves.toBeUndefined()
	...
	it('raises a 404 when no imprenditore carries that id', async () => {
		...
		expect(await rejection(funImprenditoreUpdatePreferenze(_id, true, true, 'DATI'))).toEqual({
			message: 'Oops',
			http: { status: 404 },
			description: 'imprenditore non trovato'
		})
...
describe('imprenditoriStatsDb', () => {
	it('counts every imprenditore, deleted ones included', async () => {
		countDocuments.mockResolvedValueOnce(7)

		await expect(imprenditoriStatsDb()).resolves.toBe(7)
