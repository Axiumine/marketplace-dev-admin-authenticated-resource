...
describe('instrument', () => {
	...
	it('hands Sentry the DSN and its two reporting options — no transport of its own', async () => {
		await importInstrument(DSN)

		// Exact-argument, not a subset match: a transport option added back later has to fail here.
		expect(init).toHaveBeenCalledExactlyOnceWith({ dsn: DSN, enableLogs: true, sendDefaultPii: true })
		expect(Object.keys(init.mock.calls[0][0] as object)).toStrictEqual(['dsn', 'enableLogs', 'sendDefaultPii'])
	})
